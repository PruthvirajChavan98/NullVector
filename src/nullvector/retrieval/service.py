...
class RetrievalService:
        ...
        *,
        ...
        if not candidates:
            if corpus is None:
                ...
                return ()
            used_widening = True
            candidates, widening_reason = self._widen_candidates(corpus=corpus, plan=plan)
        if not candidates:
            log_event(
                self._logger,
                "RetrievalSearchCompleted",
    ...
    def _index(
        ...
        *,
        ...
        if store.supports_retrieval_unit_queries:
        ...
        *,
        ...
        if plan.visual_query:
            if plan.page_filter is not None:
                ...
                if same_page_visual:
                    return (_dedupe_units(same_page_visual), "same_page_visual")
                ...
                return (_dedupe_units(same_page_units), "same_page_text_for_visual_query")
            ...
            return (_dedupe_units(visual_units), "all_visual_units")
        ...
        if plan.table_query:
            if plan.page_filter is not None:
                ...
                if same_page_tables:
                    return (_dedupe_units(same_page_tables), "same_page_tables")
                ...
                if same_page_text:
                    return (_dedupe_units(same_page_text), "same_page_text_for_table_query")
            return (
                _dedupe_units(
                    tuple(
                        unit for unit in corpus.units if unit.unit_type is RetrievalUnitType.TABLE
                    )
                ),
                "all_table_units",
            )
        ...
        if plan.page_filter is not None:
            ...
            if same_page_units:
                return (_dedupe_units(same_page_units), "same_page_units")
        ...
        if plan.unit_types:
            ...
            if type_only_units:
                return (_dedupe_units(type_only_units), "matching_unit_types")

        return (corpus.units, "full_corpus_fallback")
