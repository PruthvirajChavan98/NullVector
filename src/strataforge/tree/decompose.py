"""Deterministic-first decomposition of large committed leaf nodes."""
...
from strataforge.tree.summarize import estimate_token_count
...
def _node_text(node: HierarchyNode, pages_by_index: dict[int, PageArtifacts]) -> str:
...
def _is_credible_subheading(candidate: Any) -> bool:
    ...
    return candidate.keep and (
        ...
        or breakdown.layout_cues_available
...
class NodeDecomposer:
        ...
        *,
        ...
        for depth in range(1, self._settings.max_decomposition_depth + 1):
            ...
            for node in list(current_nodes):
                ...
                truncated_parent, children, method = result
                next_nodes = [
    ...
    def _needs_decomposition(
        ...
        if page_length > self._settings.max_pages_per_leaf_node:
            return True
        return _token_count_for_node(node, pages_by_index) > self._settings.max_tokens_per_leaf_node
    ...
    def _decompose_node(
        ...
        *,
        node: HierarchyNode,
        pages_by_index: dict[int, PageArtifacts],
    ) -> tuple[HierarchyNode, tuple[HierarchyNode, ...], DecompositionMethod] | None:
        ...
        if len(deterministic_children) >= 2:
            ...
            return truncated_parent, deterministic_children, DecompositionMethod.DETERMINISTIC

        if self._gateway is None:
            return None

        llm_children = self._llm_children(node=node, pages=pages)
        if len(llm_children) < 2:
            return None
        truncated_parent = self._refine_parent_owned_spans(node, llm_children)
        return truncated_parent, llm_children, DecompositionMethod.LLM_ASSISTED
...
def _token_count_for_node(
    ...
    return estimate_token_count(_node_text(node, pages_by_index))
