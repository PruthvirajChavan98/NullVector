...
- **Domain Contracts:** All data structures MUST use `pydantic` v2. Models must inherit from `StrataModel` (defined in `src/strataforge/domain/models.py`) which enforces `ConfigDict(extra="forbid", frozen=True, strict=True)`.
